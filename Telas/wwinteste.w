...
&Scoped-Define ENABLED-OBJECTS imgFoto bt-carrega 
...
DEFINE FRAME fMain
     bt-carrega AT ROW 12.67 COL 35 WIDGET-ID 4
     imgFoto AT ROW 2.67 COL 25 WIDGET-ID 2
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-carrega wWin
ON CHOOSE OF bt-carrega IN FRAME fMain /* Carrega */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pCarregarImagem wWin 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pEscolherECarregar wWin 
...
DO WITH FRAME {&FRAME-NAME}:
  ...
  SYSTEM-DIALOG GET-FILE cArq
    ...
    FILTERS    "Imagens (*.jpg;*.jpeg;*.png;*.bmp;*.gif)" "*.jpg;*.jpeg;*.png;*.bmp;*.gif",
               "Todos os arquivos (*.*)" "*.*"
    MUST-EXIST USE-FILENAME.

  IF cArq <> ? AND cArq <> "" THEN
    RUN pCarregarImagem IN THIS-PROCEDURE (cArq).
END.
